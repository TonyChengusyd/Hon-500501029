...
from dscamsff_tanet import get_model   
...
msca_tanet = get_model(qkv_VFA_flag=args.qkv_VFA_flag)
msca_tanet = msca_tanet.to(device)

weight = None
criterion = SegmentationLosses(weight=weight, cuda=args.cuda).build_loss(mode='ce')
optimizer = torch.optim.SGD(msca_tanet.parameters(), lr=args.lr, momentum=0, weight_decay=args.weight_decay)
...
def train(epoch, optimizer, train_loader, loss_factor_map):
    ...
    for iteration, batch in enumerate(train_loader):
        ...
        # grad = 0
        msca_tanet.zero_grad()
        inputs = Variable(inputs)
        labels = Variable(labels)
        # print(input.shape)
        out = msca_tanet(inputs, loss_factor_map)
        out = out[0]
...
def validation(epoch, best_pred, best_pred_epoch, loss_factor_map, val_loss_factor_map, save_path):
    ...
    for iteration, batch in enumerate(val_loader):
        ...
        with torch.no_grad():
            out = msca_tanet(image, loss_factor_map)
            out_ss = out[0]
    ...
    if new_pred > best_pred:
        ...
        cur_save_path = save_path + '/dscamsff_danet_best_epoch_{}_mIOU_{}.pth'.format(best_pred_epoch, round(best_pred,4))
        torch.save(msca_tanet, cur_save_path)
