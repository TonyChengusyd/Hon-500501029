...
class Spatial_Attention(nn.Module):
    ...
    def forward(self, qkv, H, W, mask=None):
        ...
        if self.position_bias:
            ...
            relative_position_bias = pos[self.relative_position_index.view(-1)].view(self.H_sp * self.W_sp,
                                                                                     self.H_sp * self.W_sp, -1)
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, reso=64, split_size=[8, 8], shift_size=[1, 2], qkv_bias=False, qk_scale=None,
        ...
        self.attns = nn.ModuleList([
            Spatial_Attention(dim // 2, idx=i, split_size=split_size, num_heads=num_heads // 2, dim_out=dim // 2,
                              ...
                              position_bias=True) for i in range(self.branch_num)])
        ...
        self.dwconv = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim),
                                    nn.BatchNorm2d(dim), nn.GELU())
        self.channel_interaction = nn.Sequential(nn.AdaptiveAvgPool2d(1),
                                                 nn.Conv2d(dim, dim // 8, kernel_size=1),
                                                 nn.GroupNorm(num_groups=1, num_channels=dim//8), nn.GELU(),
                                                 nn.Conv2d(dim // 8, dim, kernel_size=1))
        self.spatial_interaction = nn.Sequential(nn.Conv2d(dim, dim // 16, kernel_size=1), nn.GroupNorm(num_groups=1, num_channels=dim // 16),
                                                 nn.GELU(), nn.Conv2d(dim // 16, 1, kernel_size=1))
    ...
    def calculate_mask(self, H, W):
        ...
        h_slices_0 = (slice(0, -self.split_size[0]), slice(-self.split_size[0], -self.shift_size[0]),
                      slice(-self.shift_size[0], None))
        w_slices_0 = (slice(0, -self.split_size[1]), slice(-self.split_size[1], -self.shift_size[1]),
                      slice(-self.shift_size[1], None))

        h_slices_1 = (slice(0, -self.split_size[1]), slice(-self.split_size[1], -self.shift_size[1]),
                      slice(-self.shift_size[1], None))
        w_slices_1 = (slice(0, -self.split_size[0]), slice(-self.split_size[0], -self.shift_size[0]),
                      slice(-self.shift_size[0], None))
        cnt = 0
        for h in h_slices_0:
            for w in w_slices_0:
    ...
    def forward(self, x, H, W):
        ...
        # temp = nn.AdaptiveAvgPool2d(1)(conv_x)
        temp = self.channel_interaction(conv_x)
...
class Adaptive_Channel_Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.dwconv = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim),
                                    nn.BatchNorm2d(dim), nn.GELU())
        self.channel_interaction = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(dim, dim // 8, kernel_size=1),
                                                 nn.GroupNorm(num_groups=1,num_channels=dim//8), nn.GELU(),
                                                 nn.Conv2d(dim // 8, dim, kernel_size=1), )
        self.spatial_interaction = nn.Sequential(nn.Conv2d(dim, dim // 16, kernel_size=1),
                                                 nn.GroupNorm(num_groups=1, num_channels=dim // 16),
                                                 nn.GELU(), nn.Conv2d(dim // 16, 1, kernel_size=1))
...
class DATB(nn.Module):
    def __init__(self, dim, num_heads, reso=64, split_size=[2, 4], shift_size=[1, 2], expansion_factor=4.,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, rg_idx=0, b_idx=0):
        super().__init__()
        ...
        if b_idx % 2 == 0:
            # DSTB
            self.attn = Adaptive_Spatial_Attention(dim, num_heads=num_heads, reso=reso, split_size=split_size,
                                                   shift_size=shift_size, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                                   drop=drop, attn_drop=attn_drop, rg_idx=rg_idx, b_idx=b_idx)
        else:
            # DCTB
            self.attn = Adaptive_Channel_Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
...
class ResidualGroup(nn.Module):
    ...
    def __init__(self, dim, reso, num_heads, split_size=[2, 4], expansion_factor=4., qkv_bias=False, qk_scale=None,
                 drop=0., attn_drop=0., drop_paths=None, act_layer=nn.GELU,
                 norm_layer=nn.LayerNorm, depth=2, use_chk=False, resi_connection='1conv', rg_idx=0):
        super().__init__()
        self.use_chk = use_chk
        self.reso = reso

        self.blocks = nn.ModuleList([
            DATB(dim=dim, num_heads=num_heads, reso=reso, split_size=split_size,
                 shift_size=[split_size[0] // 2, split_size[1] // 2], expansion_factor=expansion_factor,
                 qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop, drop_path=drop_paths[i],
...
if __name__ == '__main__':
    ...
    x_size = [height, width]
    num_heads = 8
    net = DATB(dim=dims, num_heads=num_heads,b_idx=1).to('cuda')
    input_x = torch.randn((1, height*width, dims)).cuda()
    output_x = net(input_x, x_size)

    print("The shape of input: ", input_x.shape)
    print("The shape of output: ", output_x.shape)
